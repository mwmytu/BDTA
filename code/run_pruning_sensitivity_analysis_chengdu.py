...
def normalize_task_priorities(tasks):
    """
    O4 修复：对当日任务列表的 priority_score 做 min-max 归一化，
    映射到 [0, 1]，与 taxi composite_score / reputation 量纲一致。

    注意：本脚本使用纯匈牙利算法，不含 GA/CMAO，
    因此不需要区分 raw/normalized 两套用途，
    直接将 priority_score 整体替换为归一化值即可。
    原始值同时保留在 priority_score_raw 以备查验。
    """
    if not tasks:
        return tasks
...
class TaskAllocator:
    ...
    def _calculate_fitness(self, taxi, task):
        """
        ★ O4 修复后：task.get('priority_score') 已经是归一化后的 [0,1] 值，
          与 composite_score、daily_reputation 量纲一致。
        """
    ...
    def allocate_tasks(self, taxis, tasks, pruning_ratio):
        ...
        high_tasks        = [t for t in tasks
                             if t['priority_score'] >= threshold]
        normal_tasks      = [t for t in tasks
                             if t['priority_score'] <  threshold]
        all_allocs        = []

        if high_tasks:
            allocs_high = self._hungarian_kernel(taxis, high_tasks,
                                                  top_p_ratio=1.0)
            if allocs_high:
    ...
    def _hungarian_kernel(self, taxis, tasks,
        ...
        if len(expanded) * len(tasks) > 6000 * 6000:
            import random
...
if __name__ == '__main__':
    all_results = []
    output_dir  = 'bdta_sensitivity_pruning_results'
    ...
    for ratio in tqdm(PRUNING_LEVELS, desc="Varying Pruning Ratio"):
        ...
        for date in tqdm(filtered_date_range,
                          desc=f"Ratio={ratio}", leave=False):
            ...
            if allocations:
                ...
                daily_metrics.append({
                    'avg_allocation_quality': df['allocation_quality'].mean(),
