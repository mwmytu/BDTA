...
class TaskAllocator:
    def __init__(self, scenario_name, is_bj=False, mtp=5):
...
def _print_c_summary(c_value, c_results, dataset_label):
    if not c_results:
        return
...
def run_experiment(is_beijing: bool):
    if is_beijing:
        ...
        results_fn      = 'beijing_capacity_combined.csv'
        output_dir      = 'beijing_capacity_combined_results'
        cwd_change      = None
    else:
        dl              = "Chengdu"
        ...
        results_fn      = 'chengdu_capacity_combined.csv'
        output_dir      = 'chengdu_capacity_combined_results'
    ...
    for c_val in tqdm(CAPACITY_LEVELS, desc=f"[{dl}] 容量 c_value"):
        c_results = []
        ...
        for sn in SCENARIOS_TO_RUN:
            ...
            for date in tqdm(fd,
                ...
                if sn == 'QITA':
                    ...
                    td['composite_score'] = (
                        (1.0 - FIXED_W_TASK) * td['daily_reputation']
                        + FIXED_W_TASK * td['wtcs'])
                ...
                ap  = {'w_task': FIXED_W_TASK, 'slot_reachability': slr}
            ...
            if daily_metrics_list:
                ...
                avg['c_value']        = c_val
                avg['w_task']         = FIXED_W_TASK
                avg['dataset']        = dl
                all_results.append(avg)
                c_results.append(avg)

        _print_c_summary(c_val, c_results, dl)
        print()
...
if __name__ == '__main__':
    ...
    if all_dfs:
        ...
        merge_dir = 'capacity_combined_results'
        os.makedirs(merge_dir, exist_ok=True)
        out_path  = os.path.join(merge_dir, 'capacity_combined_all.csv')
